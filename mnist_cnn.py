...
def interence(input_tensor, train, regularizer):
    with tf.variable_scope('layer1-conv'):
        ...
        b = tf.get_variable(
            'b', shape=[CONV1_DEEP], initializer=tf.constant_initializer(0.0))
        # filter shape is :[filter_height, filter_width, in_channels, out_channels]
        # input tensor shape is:[batch, in_height, in_width, in_channels]
        # `strides = [1, stride, stride, 1]`.
        # return [batch, height, width, channels].
    # 卷积运算
        conv1 = tf.nn.conv2d(input_tensor, w, strides=[
    ...
    with tf.variable_scope('layer3-conv'):
        w = tf.get_variable('w', [CONV2_SIZE, CONV2_SIZE, CONV1_DEEP, CONV2_DEEP],
                            initializer=tf.truncated_normal_initializer(stddev=0.1))
        b = tf.get_variable(
            'b', shape=[CONV2_DEEP], initializer=tf.constant_initializer(0.0))
    ...
    with tf.variable_scope('layer5-fc1'):
        ...
        fc1_b = tf.get_variable(
            'b', shape=[FC_SIZE], initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope('layer6-fc2'):
        fc2_w = tf.get_variable('w', shape=[
                                FC_SIZE, NUM_LABEL], initializer=tf.truncated_normal_initializer(stddev=0.1))
        try:
            if regularizer != None:
        ...
        fc2_b = tf.get_variable(
            'b', shape=[NUM_LABEL], initializer=tf.constant_initializer(0.1))
        # 最后一层的输出，不需要加入激活函数
